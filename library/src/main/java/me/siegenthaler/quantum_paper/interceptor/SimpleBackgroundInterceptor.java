 ...
 * that only requires to tint the background.
 */
public abstract class SimpleBackgroundInterceptor implements QuantumResources.Interceptor {
    /**
     ...
     */
    public SimpleBackgroundInterceptor(int style) {
    ...
    public View getWidget(QuantumResources manager, Context context, AttributeSet attributes) {
        final View instance = instance(context, attributes, mDefaultStyle);
        final TypedArray array = context.obtainStyledAttributes(attributes, TINT_ATTRS, mDefaultStyle, 0);
        try {
            instance.setBackgroundResource(array.getResourceId(0, 0));
     ...
     */
    protected abstract View instance(Context context, AttributeSet attributes, int defStyle);
}
